...
When the tool schema exposes `project_root`, include the absolute path of the user's current workspace/repository. This keeps memories attached to the intended project even when the MCP server process is launched from a plugin cache directory.
